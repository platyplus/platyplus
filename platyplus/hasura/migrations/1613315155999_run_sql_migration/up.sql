CREATE OR REPLACE VIEW "metadata"."relationship_mapping" AS 
 SELECT r.table_schema,
    ...
    r.table_schema || '.' || r.table_name AS table_id,
    r.table_schema || '.' || r.table_name || '.' || COALESCE(manual.key, flat.from_column, pk.column_name) AS column_id,
    COALESCE(btrim((((((r.rel_def)::json -> 'manual_configuration'::text) -> 'remote_table'::text) -> 'schema'::text))::text, '"'::text), flat.ref_schema, btrim((((((r.rel_def)::json -> 'foreign_key_constraint_on'::text) -> 'table'::text) -> 'schema'::text))::text, '"'::text)) || '.' || COALESCE(btrim((((((r.rel_def)::json -> 'manual_configuration'::text) -> 'remote_table'::text) -> 'name'::text))::text, '"'::text), flat.ref_table, btrim((((((r.rel_def)::json -> 'foreign_key_constraint_on'::text) -> 'table'::text) -> 'name'::text))::text, '"'::text)) as remote_table_id,
    COALESCE(btrim((((((r.rel_def)::json -> 'manual_configuration'::text) -> 'remote_table'::text) -> 'schema'::text))::text, '"'::text), flat.ref_schema, btrim((((((r.rel_def)::json -> 'foreign_key_constraint_on'::text) -> 'table'::text) -> 'schema'::text))::text, '"'::text)) || '.' || COALESCE(btrim((((((r.rel_def)::json -> 'manual_configuration'::text) -> 'remote_table'::text) -> 'name'::text))::text, '"'::text), flat.ref_table, btrim((((((r.rel_def)::json -> 'foreign_key_constraint_on'::text) -> 'table'::text) -> 'name'::text))::text, '"'::text)) || '.' || COALESCE(btrim((manual.value)::text, '"'::text), flat.to_column, btrim(((((r.rel_def)::json -> 'foreign_key_constraint_on'::text) -> 'column'::text))::text, '"'::text)) as remote_column_id,
    r.table_schema || '.' || r.table_name || '.' || r.rel_name || '.' || COALESCE(manual.key, flat.from_column, pk.column_name) as id
